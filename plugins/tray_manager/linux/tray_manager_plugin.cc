...
static void _update_menu_labels(GtkWidget* menu_widget, FlValue* args) {
  ...
  gint gtk_count = g_list_length(children);
  gint min_count = item_count < gtk_count ? item_count : gtk_count;

  for (gint i = 0; i < min_count; i++) {
    FlValue* item_value = fl_value_get_list_value(items_value, i);
    GtkWidget* item = GTK_WIDGET(g_list_nth_data(children, i));

    const char* type =
        fl_value_get_string(fl_value_lookup_string(item_value, "type"));
    if (strcmp(type, "separator") == 0) continue;

    FlValue* submenu_value = fl_value_lookup_string(item_value, "submenu");
    if (submenu_value != nullptr && GTK_IS_MENU_ITEM(item)) {
      GtkWidget* sub = gtk_menu_item_get_submenu(GTK_MENU_ITEM(item));
      if (sub != nullptr) {
        _update_menu_labels(sub, submenu_value);
        continue;
      }
    }
    ...
    if (strcmp(type, "checkbox") == 0 && GTK_IS_CHECK_MENU_ITEM(item)) {
      FlValue* checked_value = fl_value_lookup_string(item_value, "checked");
      if (checked_value != nullptr) {
        const bool checked = fl_value_get_bool(checked_value);
        g_signal_handlers_block_matched(
            item, G_SIGNAL_MATCH_FUNC, 0, 0, nullptr,
            (gpointer)_on_activate, nullptr);
...
static FlMethodResponse* set_context_menu(TrayManagerPlugin* self,
                                          FlValue* args) {
  FlValue* keep_open_value = fl_value_lookup_string(args, "keepMenuOpen");
  bool keep_menu_open =
